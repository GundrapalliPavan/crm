...
import { useCreateLead } from '@/features/leads/useLeads';
...
/** Create Lead (MOBILE_PRD.md section 7.3). Mirrors login.tsx's Controller-per-field pattern. */
  ...
  async function onSubmit(values: LeadFormValues) {
    try {
      const lead = await createLead.mutateAsync({
        ...
        // Mobile has no assign UI (Field Sales Executive lacks lead.assign)
        // and no other route to it, so a lead created here is implicitly
        // claimed by its creator - otherwise it would be invisible to them
        // in the "My Leads" list they just created it from.
        assignedTo: user?.id,
        notes: values.notes || undefined,
      });
      router.replace({ pathname: '/leads/[id]', params: { id: lead.id } });
    } catch (error) {
      ...
      setError('firstName', { message: apiError?.message ?? 'Unable to create lead. Please try again.' });
    }
  }
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        >
          {isSubmitting ? <ActivityIndicator color="#ffffff" /> : <Text style={styles.submitText}>Create Lead</Text>}
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  );
